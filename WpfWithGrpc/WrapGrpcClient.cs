...
namespace WpfWithGrpc
{
    ...
    public class WrapGrpcClient
    {
        ...
        {
            var reply = 
                client.SayHelloAsync(new HelloRequest { Name = nameof(WrapGrpcClient) });
        ...
        public Task SubscribeReceivePositionOfRectangles(CancellationToken cancellationToken)
        {
            ...
            var streamReceivePositionOfRectangles = 
                client.SubscribeRectPositionReply(
            ...
            return
                taskReceivePositionOfRectangles;
        }

        /// <summary>
        /// Событие увдомляющее о поступлении данных попозициям прямоугольников. 
        /// </summary>
        public event EventHandler<PositionRectReplyEx> EvReceivePositionOfRectangles;
    }
}
